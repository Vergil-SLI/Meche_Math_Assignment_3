function truncation_error(mode)
    if mode == 0
        ...
        % ALEX! could you plot the comparison (similar to above) for explicit_midpoint pls*****
    end
    ...
    if mode == 1
        ...
        lgd = legend('forward euler', 'explicit midpoint', 'numerical solution'); % need the 2 implicit ones still
        lgd.Location = "southeast";
        xlabel("t")
        ylabel("X Value")
    end
    ...
    % % _____________________________________________________________________
    % % Global truncation error between the 2 explicit methods 
    if mode == 3
    tspan = [0, pi/6];
    h = logspace(-5, 1, 100);
